...
def fetch_batch_data(tickers: List[str]) -> List[Dict]:
    # executor.map preserves the input order, so every table/PDF remains identical
    # to the original implementation while independent network waits overlap.
    clean_tickers = [ticker for ticker in tickers if ticker]
    if not clean_tickers:
        return []
    workers = min(MAX_FETCH_WORKERS, len(clean_tickers))
...
with st.sidebar:
    ...
    if st.button("📌 使用說明 / instructions", width="stretch"):
    ...
    if loading:
        ...
        st.button("🚀 更新結果 / refresh", width="stretch", disabled=True)
    else:
        # 使用 form 來支援 ENTER 鍵
        with st.form(key="ticker_form"):
            u_input = st.text_input("輸入代號 / stock name", value=st.session_state.current_ticker).upper()
            submit_button = st.form_submit_button("🚀 更新結果 / refresh", width="stretch")
            if submit_button:
    ...
    if st.button("🔄 更新同業名單 / related industry stocks", width="stretch", disabled=loading):
        with st.spinner("獲取中..."):
            st.session_state.current_industry_stocks = get_industry_stocks(selected_industry)
            st.session_state.current_industry_data = fetch_batch_data(st.session_state.current_industry_stocks)
    ...
    if loading:
        ...
        cancel_button = st.button("❌ 取消載入", width="stretch")
        if cancel_button:
            st.session_state.loading_industry_rankings = False
            st.session_state.industry_rankings_loading_started = False
            st.rerun()
        try:
            ...
            st.session_state.loading_industry_rankings = False
            st.session_state.industry_rankings_loading_started = False
            if st.button("🔄 重試載入", width="stretch"):
                st.rerun()
    else:
        if st.button("🚀 載入行業排名 / Load Industry Rankings", width="stretch"):
    ...
    if st.session_state.get('industry_loaded', False) and st.session_state.industry_scores:
        ...
        if st.button("🗑️ 清除行業數據", width="stretch"):
            st.session_state.industry_loaded = False
            st.session_state.industry_scores = None
            st.cache_data.clear()
            st.rerun()
...
if st.session_state.right_panel_mode == "stock":
    ...
    if main_data:
        ...
        if show_advanced:
            ...
            with tab2:
                if volume_rvol_img:
                    st.image(volume_rvol_img, width="stretch")
                else:
                    st.warning("無法生成成交量圖")
            with tab3:
                if dmi_img:
                    st.image(dmi_img, width="stretch")
                else:
                    st.warning("無法生成 DMI 圖表")
            with tab4:
                if rsi_img:
                    st.image(rsi_img, width="stretch")
                else:
                    st.warning("無法生成 RSI 圖表")
            with tab5:
                if macd_img:
                    st.image(macd_img, width="stretch")
                else:
                    st.warning("無法生成 MACD 圖表")
            with tab6:
                if cci_img:
                    st.image(cci_img, width="stretch")
                else:
                    st.warning("無法生成 CCI 圖表")
            with tab7:
                if bb_img:
                    st.image(bb_img, width="stretch")
                else:
            ...
            with col1:
                ...
                st.download_button(label="📥 導出 CSV", data=csv_data, file_name=f"{main_data['代號']}_report.csv", mime="text/csv", width="stretch")
            with col2:
                if st.button("📄 生成 PDF 報告", width="stretch"):
                    with st.spinner("生成中..."):
                        ...
                        st.download_button(label="✅ 下載 PDF", data=pdf_bytes, file_name=f"{main_data['代號']}_report.pdf", mime="application/pdf", key="pdf_download", width="stretch")
                        st.success("PDF 已生成！")
            with col3:
                if st.button("📰 美股日報", width="stretch"):
                    with st.spinner("Generating daily report..."):
                        daily_report_pdf = generate_market_report("Daily Report (1-Day Change)", 1)
                        st.download_button(label="✅ Download Daily Report", data=daily_report_pdf, file_name=f"US_Stock_Daily_Report_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf", key="daily_report_download", width="stretch")
                        st.success("Daily report generated!")
            with col4:
                if st.button("📅 美股週報", width="stretch"):
                    with st.spinner("Generating weekly report..."):
                        weekly_report_pdf = generate_market_report("Weekly Report (5-Day Change)", 5)
                        st.download_button(label="✅ Download Weekly Report", data=weekly_report_pdf, file_name=f"US_Stock_Weekly_Report_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf", key="weekly_report_download", width="stretch")
                        st.success("Weekly report generated!")
            with col5:
                if st.button("📆 美股月報", width="stretch"):
                    with st.spinner("Generating monthly report..."):
                        monthly_report_pdf = generate_market_report("Monthly Report (20-Day Change)", 20)
                        st.download_button(label="✅ Download Monthly Report", data=monthly_report_pdf, file_name=f"US_Stock_Monthly_Report_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf", key="monthly_report_download", width="stretch")
...
# 模式2：同业对比
    ...
    if st.session_state.current_industry_data:
        ...
        if st.button("🔙 返回個股分析", width="stretch"):
            st.session_state.right_panel_mode = "stock"
            st.rerun()
    else:
